...
export function parseOpenApiJson(obj: Record<string, unknown>): ParsedInfo {
  ...
  const servers    = ((obj.servers   as unknown[]) ?? []) as Array<Record<string, unknown>>;
  ...
  const tags     = ((obj.tags as unknown[]) ?? []) as Array<Record<string, unknown>>;
  const tagNames = tags.map(t => String(t.name ?? '').toLowerCase());
  let category: string | undefined;
  if      (tagNames.some(t => t.includes('creat') || t.includes('submit'))) category = 'Invoice Creation';
  else if (tagNames.some(t => t.includes('valid')))                          category = 'Validation';
  else if (tagNames.some(t => t.includes('transmit') || t.includes('send'))) category = 'Transmission';
  else if (tagNames.some(t => t.includes('archiv') || t.includes('store'))) category = 'Archiving';
...
export function parseWsdl(xmlDoc: Document): ParsedInfo {
  const svc  = xmlDoc.getElementsByTagNameNS('http://schemas.xmlsoap.org/wsdl/', 'service')[0]
            ?? xmlDoc.querySelector('service');
  const addr = xmlDoc.getElementsByTagNameNS('http://schemas.xmlsoap.org/wsdl/soap/', 'address')[0]
            ?? xmlDoc.getElementsByTagNameNS('http://schemas.xmlsoap.org/wsdl/soap12/', 'address')[0]
