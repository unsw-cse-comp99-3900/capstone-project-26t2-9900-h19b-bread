...
export function updateVersion(
  ...
  return apiPut<VersionDetail>(
    `/api/apis/${apiId}/versions/${versionId}`,
    data,
  );
}
...
export function fromBackendAuth(method: string | null | undefined): string {
  ...
  return (
    AUTH_FROM_BACKEND[method] ??
    AUTH_FROM_BACKEND[method.toUpperCase()] ??
    method
  );
}
...
export function fromBackendCategory(
  ...
  if (
    capability &&
    (capability in CATEGORY_TO_BACKEND ||
      ["Invoice Creation", "Validation", "Transmission", "Archiving"].includes(
        capability,
      ))
  ) {
    return capability;
...
export function detectSpecType(
  ...
  if (
    protocol.toUpperCase() === "SOAP" ||
    /<definitions[\s>]|wsdl:/i.test(trimmed)
  ) {
    return "WSDL";
  }
  try {
...
export function nextVersionNumber(versions: VersionSummary[]): string {
  ...
  for (const version of versions) {
    const match = version.version_number
      .trim()
      .match(/^(v?)(\d+)(?:\.(\d+))?$/i);
    if (!match) continue;
    const candidate = {
